...
This remains conservative: opaque naming alone does nothing, an unrelated install tree stays Normal, and independent longer companions such as Editor/Treehouse/Visualize-style entries are unaffected. Restrictive entries keep their own residual identity token so explicit searches can still re-admit them. SearchEngine now treats an exact already-cached distinctive token as explicit identity for any entry, while `StrongMatchOnly` entries also allow a distinctive-token prefix. This makes two-character opaque identities such as generic `Q7`/`Z5` directly searchable without restoring broad family recall. This is cache-only text matching; no role inference or I/O moves into the keystroke path.

Provider Cache advances to schema 20. ResultRanking and Classic UI/geometry are unchanged, and the keystroke path remains I/O-free. Windows fixed FileVersion/ProductVersion is `0.8.0.208`.
...
The Classic operation Hint is no longer a raw 14-pixel Win32 Edit pretending to be the Delphi/VCL TEdit. It is now a dedicated **owner-drawn overlay** using the original **-13 SimSun** typography, right alignment, true vertical centering and ellipsis. Its left edge dynamically follows the measured query text plus a safety gap; when fewer than **104 logical pixels** remain the Hint hides instead of covering the user's input.
