...
- v0.8.0-alpha.5.49 begins the Classic technical closeout: ordinary searches stop deep-copying the complete command catalog, normalized query state is reused across matching, dynamic file-stem matching becomes allocation-free, the pinyin cache is bounded, and real Win32 GDI/USER/handle lifecycle soak coverage is added without changing Classic behavior or schemas.
- next desktop step: validate alpha.5.49 memory/resource plateaus and search responsiveness on the real machine, then freeze Classic technically before Modern Compact refinement.
