...
- v0.8.0-alpha.5.39 closes the family/distinctive admission boundary: StrongMatchOnly distinctive re-admission rejects queries contained wholly by the cached catalog family, while exact residual identity and family-plus-residual intent remain reachable; the guard is cache-only, schema remains 20, and no product vocabulary is added
- next relevance step: validate alpha.5.39 on real family-vs-explicit intent; if clean, close Catalog Admission and move to intent-stable usage ranking
