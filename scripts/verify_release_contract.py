...
if version in ("0.8.0-alpha.5.43", "0.8.0-alpha.5.44", "0.8.0-alpha.5.45", "0.8.0-alpha.5.46"):
    ...
    for name, expected in expected_schemas.items():
        ...
        if actual != expected:
            fail(f"v0.8 alpha.5.43 {name}={actual}, expected {expected}")

    if cpp_int("src/core/ProviderCache.cpp", "kProviderCacheSchemaVersion") != 22:
        fail("v0.8 alpha.5.43 must use Provider Cache schemaVersion 22")
    ...
    for token in (
        ...
        if token not in role_cpp:
            fail(f"v0.8 alpha.5.43 opaque topology implementation missing: {token}")
    ...
    for token in (
        ...
        if token not in role_tests:
            fail(f"v0.8 alpha.5.43 role regression missing: {token}")
    ...
    for token in (
        ...
        if token not in search_cpp + search_tests:
            fail(f"v0.8 alpha.5.43 multi-token distinctive regression missing: {token}")
    ...
    for token in (
        ...
        if token not in search_cpp:
            fail(f"v0.8 alpha.5.43 intent usage implementation missing: {token}")
    ...
    for token in (
        ...
        if token not in read("src/core/RelevancePolicy.cpp"):
            fail(f"v0.8 alpha.5.43 bounded ranking missing: {token}")

    for token in ("oneLaunch", "repeated", "capped"):
        if token not in search_tests:
            fail(f"v0.8 alpha.5.43 usage regression missing: {token}")

    for token in (
        "PackagedFamilyForAppPaths(",
        "HasPackagedFamily(",
    ):
        if token not in read("src/core/CommandMerge.cpp"):
            fail(f"v0.8 alpha.5.43 packaged merge missing: {token}")
    for token in ("apppath:stub", "aumid:acme.shopcenter_abc123!app"):
        if token not in read("tests/CommandMergeTests.cpp"):
            fail(f"v0.8 alpha.5.43 packaged regression missing: {token}")
    for token in ("queryLaunches", "kMaxQueriesPerCommand", "QueryKey(query)"):
        if token not in read("src/core/UsageStore.cpp") + read("src/core/SearchEngine.hpp"):
            fail(f"v0.8 alpha.5.43 usage persistence missing: {token}")
    for token in ("pendingNumericIntent_.query", "snapshotQuery"):
        if token not in read("src/ui/LauncherWindow.cpp"):
            fail(f"v0.8 alpha.5.43 numeric query snapshot missing: {token}")
    ...
    for forbidden in (
        ...
        if forbidden in role_cpp.lower() or forbidden in search_cpp.lower():
            fail(f"v0.8 alpha.5.43 product-specific production rule found: {forbidden}")
    ...
    for token in (
        ...
        if token not in update_tests:
            fail(f"v0.8 alpha.5.43 update-policy coverage missing: {token}")
    ...
    for token in (
        ...
        if token not in desktop_validation:
            fail(f"v0.8 alpha.5.43 desktop checklist missing: {token}")
    ...
    for token in (
        ...
        if token not in readme:
            fail(f"v0.8 alpha.5.43 README missing: {token}")

    if "## 0.8.0-alpha.5.43" not in changelog:
        fail("v0.8 alpha.5.43 changelog entry missing")

    if "v0.8.0-alpha.5.43 separates ranking history" not in roadmap:
        fail("v0.8 alpha.5.43 roadmap entry missing")
    ...
    if any(token in ranking_cpp for token in (
        ...
        fail("v0.8 alpha.5.43 catalog inference leaked into ResultRanking")
...
if version == "0.8.0-alpha.5.42":
    ...
    for name, expected in expected_schemas.items():
        actual = cpp_int("src/core/ConfigIO.hpp", name)
        if actual != expected:
            fail(f"v0.8 alpha.5.42 {name}={actual}, expected {expected}")

    if cpp_int("src/core/ProviderCache.cpp", "kProviderCacheSchemaVersion") != 22:
        fail("v0.8 alpha.5.42 must use Provider Cache schemaVersion 22")
    ...
    for token in (
        ...
        if token not in role_cpp:
            fail(f"v0.8 alpha.5.42 opaque topology implementation missing: {token}")
    ...
    for token in (
        ...
        if token not in role_tests:
            fail(f"v0.8 alpha.5.42 role regression missing: {token}")
    ...
    for token in (
        ...
        if token not in search_cpp + search_tests:
            fail(f"v0.8 alpha.5.42 multi-token distinctive regression missing: {token}")
    ...
    for token in (
        ...
        if token not in search_cpp:
            fail(f"v0.8 alpha.5.42 intent usage implementation missing: {token}")
    ...
    for token in (
        ...
        if token not in read("src/core/RelevancePolicy.cpp"):
            fail(f"v0.8 alpha.5.42 bounded ranking missing: {token}")

    for token in ("oneLaunch", "repeated", "capped"):
        if token not in search_tests:
            fail(f"v0.8 alpha.5.42 usage regression missing: {token}")
    ...
    for forbidden in (
        ...
        if forbidden in role_cpp.lower() or forbidden in search_cpp.lower():
            fail(f"v0.8 alpha.5.42 product-specific production rule found: {forbidden}")
    ...
    for token in (
        ...
        if token not in config_tests:
            fail(f"v0.8 alpha.5.42 Provider Cache regression missing: {token}")

    for token in (
        "FILEVERSION 0,8,0,212",
        "PRODUCTVERSION 0,8,0,212",
        "0.8.0-alpha.5.42",
    ):
        if token not in resources:
            fail(f"v0.8 alpha.5.42 resource version missing: {token}")

    if 'version="0.8.0.212"' not in manifest:
        fail("v0.8 alpha.5.42 manifest fixed version must be 0.8.0.212")
    ...
    for token in (
        ...
        if token not in update_tests:
            fail(f"v0.8 alpha.5.42 update-policy coverage missing: {token}")
    ...
    for token in (
        ...
        if token not in desktop_validation:
            fail(f"v0.8 alpha.5.42 desktop checklist missing: {token}")
    ...
    for token in (
        ...
        if token not in readme:
            fail(f"v0.8 alpha.5.42 README missing: {token}")

    if "## 0.8.0-alpha.5.42" not in changelog:
        fail("v0.8 alpha.5.42 changelog entry missing")

    if "v0.8.0-alpha.5.42 adds bounded usage ranking" not in roadmap:
        fail("v0.8 alpha.5.42 roadmap entry missing")
    ...
    if any(token in ranking_cpp for token in (
        ...
        fail("v0.8 alpha.5.42 catalog inference leaked into ResultRanking")
    ...
    for asset_path, expected in frozen_assets.items():
        if git_blob_sha(asset_path) != expected:
            fail(f"v0.8 alpha.5.42 frozen Classic asset changed: {asset_path}")

    print(
        "v0.8.0-alpha.5.42 Intent-Stable Usage Ranking verified:",
        "| Provider Cache schema 22 unchanged",
        "| bounded, frequency-only comparable-intent ranking",
        "| multi-token cached distinctive identity",
        "| global short-query precision preserved",
        "| no product blacklist",
        "| Classic assets frozen",
...
if version == "0.8.0-alpha.5.41":
    ...
    for name, expected in expected_schemas.items():
        actual = cpp_int("src/core/ConfigIO.hpp", name)
        if actual != expected:
            fail(f"v0.8 alpha.5.41 {name}={actual}, expected {expected}")

    if cpp_int("src/core/ProviderCache.cpp", "kProviderCacheSchemaVersion") != 22:
        fail("v0.8 alpha.5.41 must use Provider Cache schemaVersion 22")
    ...
    for token in (
        ...
        if token not in role_cpp:
            fail(f"v0.8 alpha.5.41 opaque topology implementation missing: {token}")
    ...
    for token in (
        ...
        if token not in role_tests:
            fail(f"v0.8 alpha.5.41 role regression missing: {token}")
    ...
    for token in (
        ...
        if token not in search_cpp + search_tests:
            fail(f"v0.8 alpha.5.41 multi-token distinctive regression missing: {token}")
    ...
    for forbidden in (
        ...
        if forbidden in role_cpp.lower() or forbidden in search_cpp.lower():
            fail(f"v0.8 alpha.5.41 product-specific production rule found: {forbidden}")
    ...
    for token in (
        ...
        if token not in config_tests:
            fail(f"v0.8 alpha.5.41 Provider Cache regression missing: {token}")

    for token in (
        "FILEVERSION 0,8,0,211",
        "PRODUCTVERSION 0,8,0,211",
        "0.8.0-alpha.5.41",
    ):
        if token not in resources:
            fail(f"v0.8 alpha.5.41 resource version missing: {token}")

    if 'version="0.8.0.211"' not in manifest:
        fail("v0.8 alpha.5.41 manifest fixed version must be 0.8.0.211")
    ...
    for token in (
        ...
        if token not in update_tests:
            fail(f"v0.8 alpha.5.41 update-policy coverage missing: {token}")
    ...
    for token in (
        ...
        if token not in desktop_validation:
            fail(f"v0.8 alpha.5.41 desktop checklist missing: {token}")
    ...
    for token in (
        ...
        if token not in readme:
            fail(f"v0.8 alpha.5.41 README missing: {token}")

    if "## 0.8.0-alpha.5.41" not in changelog:
        fail("v0.8 alpha.5.41 changelog entry missing")

    if "v0.8.0-alpha.5.41 adds Common Files structural corroboration" not in roadmap:
        fail("v0.8 alpha.5.41 roadmap entry missing")
    ...
    if any(token in ranking_cpp for token in (
        ...
        fail("v0.8 alpha.5.41 catalog inference leaked into ResultRanking")
    ...
    for asset_path, expected in frozen_assets.items():
        if git_blob_sha(asset_path) != expected:
            fail(f"v0.8 alpha.5.41 frozen Classic asset changed: {asset_path}")

    print(
        "v0.8.0-alpha.5.41 Shared Common Files Corroboration verified:",
        "| Provider Cache schema 22 rebuild",
        "| shared Common Files family owner corroboration",
        "| multi-token cached distinctive identity",
        "| global short-query precision preserved",
        "| no product blacklist",
        "| Classic assets frozen",
...
if version == "0.8.0-alpha.5.38":
    ...
    for name, expected in expected_schemas.items():
        actual = cpp_int("src/core/ConfigIO.hpp", name)
        if actual != expected:
            fail(f"v0.8 alpha.5.38 {name}={actual}, expected {expected}")

    if cpp_int("src/core/ProviderCache.cpp", "kProviderCacheSchemaVersion") != 20:
        fail("v0.8 alpha.5.38 must use Provider Cache schemaVersion 20")
    ...
    for token in (
        ...
        if token not in role_cpp:
            fail(f"v0.8 alpha.5.38 opaque auxiliary implementation missing: {token}")

    for token in (
        "Northwind Q7 2026",
        "Northwind Diagnostic Utility",
        ...
        if token not in role_tests:
            fail(f"v0.8 alpha.5.38 role regression missing: {token}")

    for token in (
        "utility-opaque",
        "utility-distant-opaque",
        'L"q7"',
        'L"z5"',
        "SuiteUtility",
    ):
        if token not in search_tests:
            fail(f"v0.8 alpha.5.38 SearchEngine regression missing: {token}")
    ...
    for product in (
        ...
        if product in role_cpp.lower():
            fail(f"v0.8 alpha.5.38 product-specific production rule found: {product}")
    ...
    for token in (
        ...
        if token not in config_tests:
            fail(f"v0.8 alpha.5.38 Provider Cache regression missing: {token}")

    development_start = workflow.find("  development-release:")
    if development_start < 0:
        fail("v0.8 alpha.5.38 development-release job missing")
    ...
    for token in (
        ...
        if token not in development:
            fail(f"v0.8 alpha.5.38 dev-latest verification regressed: {token}")

    for token in (
        "FILEVERSION 0,8,0,208",
        "PRODUCTVERSION 0,8,0,208",
        "0.8.0-alpha.5.38",
    ):
        if token not in resources:
            fail(f"v0.8 alpha.5.38 resource version missing: {token}")

    if 'version="0.8.0.208"' not in manifest:
        fail("v0.8 alpha.5.38 manifest fixed version must be 0.8.0.208")
    ...
    for token in (
        ...
        if token not in update_tests:
            fail(f"v0.8 alpha.5.38 update-policy coverage missing: {token}")
    ...
    for token in (
        ...
        if token not in desktop_validation:
            fail(f"v0.8 alpha.5.38 desktop checklist missing: {token}")
    ...
    for token in (
        ...
        if token not in readme:
            fail(f"v0.8 alpha.5.38 README missing: {token}")

    if "## 0.8.0-alpha.5.38" not in changelog:
        fail("v0.8 alpha.5.38 changelog entry missing")

    if "v0.8.0-alpha.5.38 adds opaque auxiliary corroboration" not in roadmap:
        fail("v0.8 alpha.5.38 roadmap entry missing")
    ...
    for asset_path, expected in frozen_assets.items():
        if git_blob_sha(asset_path) != expected:
            fail(f"v0.8 alpha.5.38 frozen Classic asset changed: {asset_path}")

    print(
        "v0.8.0-alpha.5.38 Opaque Auxiliary Corroboration verified:",
        "| Provider Cache schema 20 rebuild",
        "| opaque identity alone is non-suppressive",
...
if version == "0.8.0-alpha.5.33":
    ...
    for name, expected in expected_schemas.items():
        actual = cpp_int("src/core/ConfigIO.hpp", name)
        if actual != expected:
            fail(f"v0.8 alpha.5.33 {name}={actual}, expected {expected}")

    if cpp_int("src/core/ProviderCache.cpp", "kProviderCacheSchemaVersion") != 15:
        fail("v0.8 alpha.5.33 must use Provider Cache schemaVersion 15")
    ...
    for token in (
        "One- and two-character ASCII queries are too ambiguous",
        "normalizedQuery.size() >= 3",
        "BoundaryPrefix",
    ):
        if token not in relevance_cpp:
            fail(f"v0.8 alpha.5.33 short-query policy missing: {token}")

    for token in (
        "shortPrecision",
        "Xbox App Admin Server",
        ...
        if token not in search_tests:
            fail(f"v0.8 alpha.5.33 short-query regression missing: {token}")
    ...
    if "normalizedQuery.size() >= 3" in search_cpp:
        fail("v0.8 alpha.5.33 short-query policy must stay centralized in RelevancePolicy")

    if "BoundaryPrefix" in ranking_cpp and "MatchKindTier" not in ranking_cpp:
        fail("v0.8 alpha.5.33 must not add ad-hoc short-query ranking hacks")
    ...
    for product in (
        ...
        if product in (relevance_cpp + start_menu_cpp).lower():
            fail(f"v0.8 alpha.5.33 product-specific production rule found: {product}")
    ...
    for token in (
        ...
        if token not in config_tests:
            fail(f"v0.8 alpha.5.33 Provider Cache regression missing: {token}")

    development_start = workflow.find("  development-release:")
    if development_start < 0:
        fail("v0.8 alpha.5.33 development-release job missing")
    development = workflow[development_start:]
    for token in (
        "cancel-in-progress: false",
        "Verify dev-latest public update contract",
        ...
        if token not in development:
            fail(f"v0.8 alpha.5.33 dev-latest verification regressed: {token}")

    for token in (
        "FILEVERSION 0,8,0,203",
        "PRODUCTVERSION 0,8,0,203",
        "0.8.0-alpha.5.33",
    ):
        if token not in resources:
            fail(f"v0.8 alpha.5.33 resource version missing: {token}")

    if 'version="0.8.0.203"' not in manifest:
        fail("v0.8 alpha.5.33 manifest fixed version must be 0.8.0.203")
    ...
    for token in (
        ...
        if token not in update_tests:
            fail(f"v0.8 alpha.5.33 update-policy coverage missing: {token}")
    ...
    for token in (
        ...
        if token not in desktop_validation:
            fail(f"v0.8 alpha.5.33 desktop checklist missing: {token}")
    ...
    for token in (
        ...
        if token not in readme:
            fail(f"v0.8 alpha.5.33 README missing: {token}")

    if "## 0.8.0-alpha.5.33" not in changelog:
        fail("v0.8 alpha.5.33 changelog entry missing")

    if "v0.8.0-alpha.5.33 separates short-query precision" not in roadmap:
        fail("v0.8 alpha.5.33 roadmap entry missing")
    ...
    for asset_path, expected in frozen_assets.items():
        if git_blob_sha(asset_path) != expected:
            fail(f"v0.8 alpha.5.33 frozen Classic asset changed: {asset_path}")

    print(
        "v0.8.0-alpha.5.33 Short Query Precision verified:",
        "| Provider Cache schema 15 rebuild",
        "| 1-2 ASCII BoundaryPrefix suppression",
        "| exact/prefix/alias/initials preserved",
        "| 3-character BoundaryPrefix preserved",
        "| resolved-target Start Menu structural surfaces",
        "| no product blacklist",
        "| dev-latest public verification preserved",
        "| Classic assets frozen",
    )
...
if version == "0.8.0-alpha.5.32":
    ...
    for name, expected in expected_schemas.items():
        actual = cpp_int("src/core/ConfigIO.hpp", name)
        if actual != expected:
            fail(f"v0.8 alpha.5.32 {name}={actual}, expected {expected}")

    if cpp_int("src/core/ProviderCache.cpp", "kProviderCacheSchemaVersion") != 14:
        fail("v0.8 alpha.5.32 must use Provider Cache schemaVersion 14")
    ...
    for token in (
        ...
        if token not in role_cpp:
            fail(f"v0.8 alpha.5.32 residual catalog evidence missing: {token}")

    for token in (
        "Acme Network Monitor 2026",
        "Acme Service Manager 2026",
        "Acme Service Host",
        "Fabrikam Studio Network Monitor 2026",
        "Fabrikam Studio License Manager 2026",
        "Fabrikam Studio Service Manager 2026",
        "Fabrikam Studio Network Designer 2026",
    ):
        if token not in role_tests:
            fail(f"v0.8 alpha.5.32 generic role regression missing: {token}")

    for token in (
        "residual-network-monitor",
        "residual-license-manager",
        "residual-service-manager",
        "residual-network-designer",
        "monitorIntent",
        "licenseIntent",
        "serviceIntent",
    ):
        if token not in search_tests:
            fail(f"v0.8 alpha.5.32 end-to-end residual search regression missing: {token}")

    if "HasContextualSuiteUtilityPhrase(" in search_cpp:
        fail("v0.8 alpha.5.32 residual role reasoning must stay out of SearchEngine")

    if "catalogVisibility" in result_ranking or "distinctiveTokens" in result_ranking:
        fail("v0.8 alpha.5.32 catalog admission must not move into ResultRanking")
    ...
    for product in forbidden_products:
        if product in production_and_role_tests:
            fail(f"v0.8 alpha.5.32 product-specific role rule/fixture found: {product}")
    ...
    for token in (
        ...
        if token not in config_tests:
            fail(f"v0.8 alpha.5.32 Provider Cache regression missing: {token}")

    development_start = workflow.find("  development-release:")
    if development_start < 0:
        fail("v0.8 alpha.5.32 development-release job missing")
    development = workflow[development_start:]
    for token in (
        "cancel-in-progress: false",
        "Verify dev-latest public update contract",
        ...
        if token not in development:
            fail(f"v0.8 alpha.5.32 dev-latest verification regressed: {token}")

    for token in (
        "FILEVERSION 0,8,0,202",
        "PRODUCTVERSION 0,8,0,202",
        "0.8.0-alpha.5.32",
    ):
        if token not in resources:
            fail(f"v0.8 alpha.5.32 resource version missing: {token}")

    if 'version="0.8.0.202"' not in manifest:
        fail("v0.8 alpha.5.32 manifest fixed version must be 0.8.0.202")
    ...
    for token in (
        ...
        if token not in update_tests:
            fail(f"v0.8 alpha.5.32 update-policy coverage missing: {token}")
    ...
    for token in (
        ...
        if token not in desktop_validation:
            fail(f"v0.8 alpha.5.32 desktop checklist missing: {token}")

    for token in (
        "v0.8.0-alpha.5.32 — Catalog Residual Evidence Completion",
        "Provider Cache advances to schema 14",
        "0.8.0.202",
    ):
        if token not in readme:
            fail(f"v0.8 alpha.5.32 README missing: {token}")

    if "## 0.8.0-alpha.5.32" not in changelog:
        fail("v0.8 alpha.5.32 changelog entry missing")

    if "v0.8.0-alpha.5.32 completes generic residual management evidence" not in roadmap:
        fail("v0.8 alpha.5.32 roadmap entry missing")
    ...
    for asset_path, expected in frozen_assets.items():
        if git_blob_sha(asset_path) != expected:
            fail(f"v0.8 alpha.5.32 frozen Classic asset changed: {asset_path}")

    print(
        "v0.8.0-alpha.5.32 Catalog Residual Evidence Completion verified:",
        "| Provider Cache schema 14 rebuild",
        "| contextual network/license/service-management utility evidence",
        "| component-role repair requires title + suite primary",
        "| standalone utilities remain Normal",
        "| independent companions preserved",
        "| SearchEngine/ResultRanking unchanged",
        "| dev-latest public verification preserved",
        "| Classic assets frozen",
...
if version == "0.8.0-alpha.5.17":
    ...
    for name, expected in expected_schemas.items():
        actual = cpp_int("src/core/ConfigIO.hpp", name)
        if actual != expected:
            fail(f"v0.8 alpha.5.17 {name}={actual}, expected {expected}")

    if cpp_int("src/core/ProviderCache.cpp", "kProviderCacheSchemaVersion") != 6:
        fail("v0.8 alpha.5.17 must rebuild generated provider cache at schemaVersion 6")
    ...
    for token in (
        ...
        if token not in candidate_h:
            fail(f"v0.8 alpha.5.17 admission explainability contract missing: {token}")

    for token in (
        "LaunchAdmissionReason::Admitted",
        "LaunchAdmissionReason::Documentation",
        "LaunchAdmissionReason::Maintenance",
        "LaunchAdmissionReason::Auxiliary",
        "LaunchAdmissionReason::UnsupportedTarget",
        "ExecutableUnknown",
    ):
        if token not in candidate_cpp:
            fail(f"v0.8 alpha.5.17 admission reason/fallback implementation missing: {token}")
    ...
    for token in (
        ...
        if token not in inspector_cpp:
            fail(f"v0.8 alpha.5.17 executable inspection fallback missing: {token}")
    ...
    for token in (
        ...
        if token not in inspector_tests:
            fail(f"v0.8 alpha.5.17 Windows shortcut fallback regression missing: {token}")

    for token in (
        "start:teamspeak3",
        "start:teamspeak6",
        "merged.commands.size() == 2",
    ):
        if token not in merge_tests:
            fail(f"v0.8 alpha.5.17 TeamSpeak merge regression missing: {token}")
    ...
    for token in (
        ...
        if token not in config_tests:
            fail(f"v0.8 alpha.5.17 provider-cache rebuild regression missing: {token}")
    ...
    for path, expected in frozen_assets.items():
        if git_blob_sha(path) != expected:
            fail(f"v0.8 alpha.5.17 frozen Classic asset changed: {path}")

    for token in (
        "FILEVERSION 0,8,0,187",
        "PRODUCTVERSION 0,8,0,187",
        "0.8.0-alpha.5.17",
    ):
        if token not in resources:
            fail(f"v0.8 alpha.5.17 resource version missing: {token}")

    if 'version="0.8.0.187"' not in manifest:
        fail("v0.8 alpha.5.17 manifest fixed version must be 0.8.0.187")
    ...
    for token in (
        ...
        if token not in update_tests:
            fail(f"v0.8 alpha.5.17 update-policy coverage missing: {token}")

    for token in (
        "v0.8.0-alpha.5.17 Executable admission recovery validation",
        "TeamSpeak 3 Client",
        "TeamSpeak.exe",
        "Provider Cache schema 6",
    ):
        if token not in desktop_validation:
            fail(f"v0.8 alpha.5.17 desktop checklist missing: {token}")
    ...
    for token in (
        ...
        if token not in readme:
            fail(f"v0.8 alpha.5.17 README missing: {token}")

    if "## 0.8.0-alpha.5.17" not in changelog:
        fail("v0.8 alpha.5.17 changelog entry missing")

    if "v0.8.0-alpha.5.17 hardens that admission boundary" not in roadmap:
        fail("v0.8 alpha.5.17 roadmap entry missing")

    print(
        "v0.8.0-alpha.5.17 Executable Admission Recovery verified:",
        "| executable-unknown fallback",
        "| explicit admission reasons",
        "| TeamSpeak dual-install merge preserved",
        "| provider cache schema 6",
        "| Classic assets frozen",
    )
    raise SystemExit(0)
